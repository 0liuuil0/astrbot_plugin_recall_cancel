...
class Main(star.Star):
    ...
    async def on_all_message(self, event: AstrMessageEvent) -> None:
        ...
        if policy == BOT_RESPONSE_POLICY_LEGACY_LAST:
            ...
            removed = await self._remove_context_bot_response(umo, recalled_msg_id, pending)
            if not removed:
                await self._schedule_deferred_safe_context_bot_cleanup(umo, recalled_msg_id, pending)
        logger.debug("[RecallCancel] Recall notice propagation left open")
        # ← on_all_message 到此结束，下面的方法必须回到类顶层缩进
    
            # ✅ 修复1: on_llm_request 必须在类顶层，不能嵌套
    ...
    async def on_llm_request(
        ...
        # ✅ 修复2: on_llm_response 必须包含 request 参数
    @filter.on_llm_response(priority=100)
    async def on_llm_response(
        self, event: AstrMessageEvent, request: ProviderRequest, response: LLMResponse
    ) -> None:
    ...
    async def on_decorating_result(
        self, event: AstrMessageEvent, result: dict  # ← 补全缺失的 result 参数
    ) -> None:
